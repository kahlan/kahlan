<?php
namespace Kahlan\Reporter;

class Tap extends OutputReporter
{
    /**
     * Store the current number of dots.
     * 
     * @var integer
     */
    protected $_counter = 0;

    /**
     * Store lines for TAP report
     *
     * @var array
     */
    protected $_lines = [];

    /**
     * Store counters for all types of tests
     *
     * @var array
     */
    protected $_counters = [
    	'success' => 0,
    	'failed' => 0,
    	'skipped' => 0
    ];

    /**
     * Callback called before any specs processing.
     *
     * @param array $params The suite params array.
     */
    public function start($params)
    {
        parent::start($params);
        $this->write("\n");
    }

    /**
     * Callback called on successful expect.
     *
     * @param object $report An expect report object.
     */
    public function pass($report = null)
    {
    	$this->_counters['success'] += 1;
        $this->_write('.');
    }

    /**
     * Callback called on failure.
     *
     * @param object $report An expect report object.
     */
    public function fail($report = null)
    {
    	$this->_counters['failed'] += 1;
        $this->_write('F', 'red');
    }

    /**
     * Callback called when an exception occur.
     *
     * @param object $report An expect report object.
     */
    public function exception($report = null)
    {
    	$this->_counters['failed'] += 1;
        $this->_write('E', 'magenta');
    }

    /**
     * Callback called on a skipped spec.
     *
     * @param object $report An expect report object.
     */
    public function skip($report = null)
    {
    	$this->_counters['skipped'] += 1;
        $this->_write('S', 'cyan');
    }

    /**
     * Callback called when a `Kahlan\IncompleteException` occur.
     *
     * @param object $report An expect report object.
     */
    public function incomplete($report = null)
    {
    	$this->_counters['skipped'] += 1;
        $this->_write('I', 'yellow');
    }

    /**
     * Callback called at the end of specs processing.
     */
    public function end($results = [])
    {
        do {
            $this->_write(' ');
        } while ($this->_counter % 80 !== 0);

        $this->write("\n");
        $this->_lines[] = "1..{$this->_counter}";

        $counter = 0;
        foreach ($results['specs'] as $type => $reports) {
            foreach ($reports as $report) {
            	$counter++;

				switch($report->type()) {
		            case "pass":
		            case "skip":
					case "incomplete":
						$this->_lines[] = $this->_formatTapReport($counter, true, $report);
		            	break;
		            case "fail":
		            	$this->_lines[] = $this->_formatTapReport($counter, false, $report);

		            	// Write down debug information
		            	$this->_lines[] = "# Actual: {$report->params()["actual"]}";
		            	$this->_lines[] = "# Expected: {$report->params()["expected"]}";
		            	break;
		            case "exception":
		            	$this->_lines[] = $this->_formatTapReport($counter, false, $report);

		            	// Write down debug information
		            	$exception = $report->exception();
		            	$this->_lines[] = '# Exception: `' . get_class($exception) .'` Code(' . $exception->getCode() . '): ';
		            	$this->_lines[] = '# Message: ' . $exception->getMessage();
		            	break;	
		        }

                if ($report->type() !== 'pass' && $report->type() !== 'skip') {
                    $this->_report($report);
                }
            }
        }

        $this->write("\n\n");
        $this->_summary($results);
        $this->_reportFocused($results);

        // Writing total information
        $this->_lines[] = "# pass {$this->_counters['success']}";
        $this->_lines[] = "# fail {$this->_counters['failed']}";
        $this->_lines[] = "# skip {$this->_counters['skipped']}";

        // Write report to file and exit
        fwrite($this->_fp, implode("\n", $this->_lines));
        fclose($this->_fp);
    }

    /**
     * Outputs the string message in the console.
     *
     * @param string       $string  The string message.
     * @param array|string $options The color options.
     */
    protected function _write($string, $options = null)
    {
        $this->write($string, $options);
        $this->_counter++;
        if ($this->_counter % 80 === 0) {
            $this->write(' ' . floor(($this->_current * 100) / $this->_total) . "%\n");
        }
    }


    protected function _formatTapReport($counter, $success, $report)
    {
    	$isOk = ($success) ? "ok" : "not ok";
    	$message = $report->file() . ": " .trim(implode(" ", $report->messages()));
    	return "{$isOk} {$counter} {$message}";
    }

}