...
trait StaticFiltering {
	 ...
	 */
	public static function applyFilter($methods, $closure) {
		$class = get_called_class();
		$closure = $closure->bindTo(null, $class);
		foreach ((array) $methods as $method) {
			static::$_methodFilters[$class][$method][] = $closure;
		}
	}
	 ...
	 * @return mixed
	 */
	protected static function _filter($method, $params, $callback) {
		$class = get_called_class();
		if (empty(static::$_methodFilters[$class][$method])) {
			static::$_methodFilters[$class][$method] = [];
		}
		$data = array_merge(static::$_methodFilters[$class][$method], [$callback]);
